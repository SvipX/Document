...
```
$sign = md5("amount={$amount}&app_id={$app_id}&bank_account={$bank_account}&bank_account_type={$bank_account_type}$bank_id={$bank_id}&bank_user_id={$bank_user_id}&bank_user_name={$bank_user_name}&city={$city}&notify_url={$notify_url}&order_no={$order_no}&ts={$ts}&key={$secret_key}")
```
...
### 通知验参示例：
```
app_id={$app_id}status={$status}order_no={$order_no}transaction_id={$transaction_id}amount={$amount}{$ts}{$secret_key}
```
签名规则参照支付接口
