...
| state                | 选填 | 商户自定义信息，原值传回 |
| ts                   | 必填 | unix格式时间戳 |
| sign                 | 必填 | 签名 |

### 签名方式(注意顺序)：
```
$sign_str = "app_id={$app_id}notify_url={$notify_url}order_no={$order_no}pay_amt={$pay_amt}pay_cur={$pay_cur}pay_type={$pay_type}return_url={$return_url}{$ts}[secret_key]";
$sign = md5($sign_str);
```

花括号里的值替换成参数的值，[secret_key]为商户秘钥（见商户后台）
对以上内容进行md5，结果为英文为小写
...
| state          |   一定有   | 回传给商户的自定义信息 |
| pay_actual_amt |   一定有   | 实际支付金额 |
| state          |   一定有   | 自定义信息，原值传回 |
| ts             |   一定有   | unix格式时间戳 |
| sign           |   一定有   | md5签名 |
...
### 签名参数：
```
$sign_str = "app_id={app_id}is_success={is_success}order_no={order_no}pay_actual_amt={pay_actual_amt}{ts}[secret_key]"
$sign = md5($sign_str);
```
花括号内的值为收到的值，[secret_key]为商户秘钥（见商户后台）
对以上内容进行md5，结果里的英文为小写
